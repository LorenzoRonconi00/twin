...
export default async function handler(
    ...
    if(req.method !== "POST"){
        return res.status(405).json({error: "Operazione non permessa"});
    }
    ...
    try {
        ...
        if(!serverId){
            return res.status(400).json({error: "ID Server mancante"});
        }

        if(!channelId){
            return res.status(400).json({error: "ID Canale mancante"});
        }
        ...
        if(!server){
            return res.status(404).json({message: "Server non trovato"});
        }
